...
export default {
  ...
  watch: {
    ...
    appMode(newMode, oldMode) {
      ...
      if (this.appMode === 'record' && !store.recorder) this.initRecorder()
  ...
  methods: {
    ...
    shaked() {
      const randomPianoMode =
        store.pianoModes[Math.floor(Math.random() * store.pianoModes.length)]
    ...
    initDraggables() {
      for (let i = 0; i < store.planets.length; i++) {
        ...
        store.planets[i].draggable = Draggable.create(`#planet-${name}`, {
          cursor: 'pointer',
          onDrag: () => {
    ...
    determineAmplitude(name) {
      ...
      const { size } = planet
      const y =
        document.querySelector(sel).transform.baseVal.getItem(0).matrix.f - size
    ...
    positionPlanetsHorizontally() {
      ...
      for (let i = 0; i < this.planets.length; i++) {
        ...
        const x =
          index * ((this.canvas.width - sunWidth) / this.planets.length) +
      ...
      for (let i = 0; i < this.planets.length; i++) {
        ...
        do {
          ...
          for (let j = 0; j < i; j++) {
            ...
            const d = Math.sqrt(Math.abs(x1 - x2) ** 2 + Math.abs(y1 - y2) ** 2)
            if (d < 2 * r1 + 2 * r2) {
              overlap = true
              break
            }
        ...
        console.log(
          `Took ${iterations} iterations for planet ${this.planets[i].name}`
        )
